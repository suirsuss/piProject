...
def setup(segment):
    ''' One-time run only code. It sets up the correct GPIO to the correct outputs'''
...
def setValue(value, segment):
    '''Input: integer and the segment. I wrote this piece of code so that we don't have to
    worry about working with binary/hex (which is necessary for the shift register)'''
